...
class FavoritesPage(context: Context) : BasePage(context) {
    ...
    private fun sortedFavoriteItems(items: List<FavoritesStore.FavoriteItem>): List<FavoritesStore.FavoriteItem> =
        items.sortedWith { left, right ->
            ...
            val titleCompare = compareNaturalTitle(leftTitle, rightTitle)
            if (titleCompare != 0) titleCompare else left.uri.compareTo(right.uri)
        }

    private data class NaturalSortToken(val text: String? = null, val number: Long? = null)

    private fun compareNaturalTitle(left: String, right: String): Int {
    ...
    private fun normalizeNaturalSortTokens(tokens: List<NaturalSortToken>): List<NaturalSortToken> {
    ...
    private fun naturalSortTokens(value: String): List<NaturalSortToken> {
        val tokens = mutableListOf<NaturalSortToken>()
        var index = 0
        while (index < value.length) {
            ...
            when {
                ch.isDigit() -> {
                    ...
                    tokens.add(NaturalSortToken(number = number))
                }
                isChineseNumberChar(ch) -> {
                    val start = index
                    while (index < value.length && isChineseNumberChar(value[index])) index++
                    tokens.add(NaturalSortToken(number = parseChineseNumber(value.substring(start, index))))
                }
                else -> {
                    val start = index
                    while (index < value.length && !value[index].isDigit() && !isChineseNumberChar(value[index])) index++
                    tokens.add(NaturalSortToken(text = value.substring(start, index).lowercase()))
                }
            }
        }
        return tokens
